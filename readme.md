...
| `LIDER` | Como `USUARIO`, mais `PATCH` em ativos só no campo **perfil de usinagem** (leve/pesado). Com a OS em **`AGUARDANDO_APROVACAO`**, aplica e preenche o checklist **`FINALIZACAO_OS`**. Fora dessa fase, preenche o checklist **`LOTO_LIDER`** (validação de liderança após o **`LOTO`** do técnico). Pode **registrar apontamentos** (`POST` em `/ordens-servico/.../apontamentos`) para mudar status como **TECNICO**; **finalizar** (`FINALIZADA`) e **cancelar** (`CANCELADA`) exigem o mesmo perfil que **ADMIN** (finalização com **`FINALIZACAO_OS`** concluído). |
...
- **Cadeia LOTO (operacional + líder):** o técnico (**`TECNICO`**, **`LUBRIFICADOR`**, **`ADMIN`**) preenche **`LOTO`**; a liderança (**`LIDER`**, **`ADMIN`**, **`DIRETORIA`**) preenche **`LOTO_LIDER`**, depois de **`LOTO`** concluído. Para **mudar o status** da OS (qualquer transição cujo novo status seja **diferente** do anterior), a API exige **ambos** concluídos, **exceto**: **`CANCELADA`** (sem exigência de checklist); **`ABERTA` → `AGENDADA`** (só agendamento, **sem** exigir a cadeia). Apontamento **sem mudança de status** (só texto) **não** exige a cadeia.
- **`ABERTA` → `CANCELADA`**: apenas **ADMIN**/**LIDER**; **sem** exigência de LOTO/LOTO_LIDER.
- Ao abrir o detalhe da OS, **`POST .../garantir-padroes-obrigatorios`** cria na OS (se ainda não existir) as execuções de **`LOTO`**, **`LOTO_LIDER`** e **`FINALIZACAO_OS`**, quando os padrões estiverem ativos. O checklist da **TAG** (preventiva) **não** exige a cadeia concluída no momento do vínculo. Enquanto a OS **não** estiver **finalizada** nem **cancelada**, **anexos** e **solicitações de peças** exigem a cadeia **LOTO + LOTO_LIDER** concluída.
- A preventiva em **`/preventivas/…/executar`** nasce em **`AGENDADA`** (com `data_agendamento`); inclui **LOTO**, **LOTO_LIDER**, **FINALIZACAO_OS** e o checklist cujo `codigo_checklist` = **TAG do ativo** (quando os padrões existem).
...
- Checklists — com OS em **`AGUARDANDO_APROVACAO`**, **só** **`FINALIZACAO_OS`** é editável; **`LOTO_LIDER`** não é editável nessa fase. **`FINALIZACAO_OS`**: **LIDER** ou **ADMIN**; **`LOTO_LIDER`**: **LIDER**, **ADMIN** ou **DIRETORIA**, com **`LOTO`** já concluído. **`TECNICO`** e **`LUBRIFICADOR`** **não** preenchem **`LOTO_LIDER`**. Demais checklists: **ADMIN**, **TECNICO** e **LUBRIFICADOR** só com **cadeia LOTO + LOTO_LIDER** concluída e OS fora da fase de só-finalização.
...
- **Mudança de status** (novo status **diferente** do anterior), exceto **`CANCELADA`**: exige checklists **`LOTO`** e **`LOTO_LIDER`** concluídos (a API acrescenta **`CHECKLIST_OK: LOTO`** e **`CHECKLIST_OK: LOTO_LIDER`**). **Exceção:** **`ABERTA` → `AGENDADA`** não exige a cadeia. Apontamento **com o mesmo status** (só descrição) **não** exige a cadeia.
- **`FINALIZADA`**: apenas **ADMIN** ou **LIDER**; checklists **`LOTO`**, **`LOTO_LIDER`** (regra acima) e **`FINALIZACAO_OS`** concluídos. A API acrescenta as marcas de checklist; ativo volta a **`OPERANDO`**.
- **`CANCELADA`**: apenas **ADMIN** ou **LIDER**; **sem** exigência de checklist.
...
| `POST` | `/ordens-servico/{work_order_id}/solicitacoes-pecas` | Registra solicitação: `quantidade`, `descricao`, `codigo_peca` (opcional). Os valores são **copiados** para a OS; alterações posteriores no cadastro de peças **não** atualizam solicitações antigas. Perfis: **ADMIN**, **TECNICO**, **LUBRIFICADOR**. Com a OS **não** terminal, exige **cadeia LOTO** (**`LOTO`** + **`LOTO_LIDER`**) concluída. |
...
| `POST` | `/preventivas/{plan_id}/executar` | **Registra execução e abre OS preventiva** (`tipo_manutencao=PREVENTIVA`, **`status=AGENDADA`**, `data_agendamento` = instante da execução) no ativo do plano, com `solicitante` = usuário autenticado; cód. OS no formato `OS-PRV-…` (único). Garante na OS os padrões **LOTO**, **LOTO_LIDER** e **FINALIZACAO_OS** (se ativos) e o checklist cuja `codigo_checklist` = **TAG do ativo** (ex.: `N002-RP-T63`) — o checklist **deve** existir no catálogo com código igual (ignorando maiúsculas) à tag, senão a API responde **422**; o ativo precisa ter TAG. O vínculo do checklist da TAG **não** exige a cadeia LOTO concluída. Atualiza `ultima_execucao` e `proxima_execucao` do plano. Resposta: `ExecutarPreventivaResponse` com `plano` e `ordem_servico`. **ADMIN**, **TECNICO**, **LUBRIFICADOR**. |
...
| Método | Caminho | Descrição |
|--------|---------|-----------|
| `GET` | `/checklists` | Lista checklists padrão. Query: `ativo` (opcional), `limit`, `offset`. **`USUARIO`**: mesma listagem (uso no detalhe da OS; gravação em execução segue `POST …/executar`). **LIDER** vê apenas os códigos **`FINALIZACAO_OS`** e **`LOTO_LIDER`**. |
...
| `POST` | `/checklists/ordens-servico/{work_order_id}/garantir-padroes-obrigatorios` | Garante na OS execução **`LOTO`**, **`LOTO_LIDER`** e **`FINALIZACAO_OS`** (padrões ativos), se a OS **não** está **`FINALIZADA`** nem **`CANCELADA`** (inclui com status **`AGENDADA`**). Resolução de padrão por `codigo_checklist` **sem** depender de maiúsculas no banco. Idempotente. Corpo vazio `{}`. Retorno: `criadas`, `checklist_executada_ids`. |
| `POST` | `/checklists/ordens-servico/{work_order_id}/executar` | Copia checklist padrão para a **OS**. **`LOTO`**: **ADMIN/TECNICO/LUBRIFICADOR**; ao aplicar **`LOTO`**, a API cria também **`LOTO_LIDER`** se o padrão existir e ainda não houver cópia. **`LOTO_LIDER`**: **LIDER/ADMIN/DIRETORIA** (com **`LOTO`** concluído). **`FINALIZACAO_OS`**: **LIDER** ou **ADMIN** com OS em **`AGUARDANDO_APROVACAO`**. Com OS em **`AGUARDANDO_APROVACAO`**, só **`FINALIZACAO_OS`** pode ser aplicado manualmente. Outros checklists: **ADMIN/TECNICO/LUBRIFICADOR** só com cadeia **LOTO+LOTO_LIDER** concluída. **`LOTO`**, **`LOTO_LIDER`** e **`FINALIZACAO_OS`**: no máximo **uma** execução por OS cada (**409** se já existir). Resposta inclui `codigo_checklist`. |
...
| `GET` | `/checklists/ordens-servico/{work_order_id}/obrigatorios-status` | Situação dos obrigatórios: **`LOTO`**, **`LOTO_LIDER`**, **`FINALIZACAO_OS`**. Em **`FINALIZACAO_OS`**, `concluido_copia_lider`: concluído e (**cópia** feita por **LIDER** ou **todas** as tarefas obrigatórias com último preenchimento por **LIDER** — ver tarefas `ultimo_preenchimento_por_id`). |
...
| `PATCH` | `/checklists/execucoes/tarefas/{task_id}` | Marca tarefa e/ou observação; grava **último preenchimento**. **`FINALIZACAO_OS`**: **LIDER** (OS em **`AGUARDANDO_APROVACAO`**) ou **ADMIN**. **`LOTO`**: **ADMIN**, **TECNICO** ou **LUBRIFICADOR**. **`LOTO_LIDER`**: **LIDER**, **ADMIN** ou **DIRETORIA**, com **`LOTO`** concluído; **não** em **`AGUARDANDO_APROVACAO`**. **Outros** checklists: **ADMIN** / **TECNICO** / **LUBRIFICADOR**, cadeia **LOTO+LOTO_LIDER** concluída; **não** em **`AGUARDANDO_APROVACAO`**. |
